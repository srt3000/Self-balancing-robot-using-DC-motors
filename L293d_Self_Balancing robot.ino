...
{ // Define outputs
  ...
  pinMode(in4_motor_right, OUTPUT); // Start serial console
  Serial.begin(115200); //BT.begin(9600); 
  delay(50); // Initiate the Wire library and join the I2C bus as a master or slave 
  Wire.begin();
}

TWBR = 12//((F_CPU / 400000L) - 16) / 2; // Set I2C frequency to 400kHz
...
// atan2 outputs the value of -Ï€ to Ï€ (radians) - seehttp://en.wikipedia.org/wiki/Atan2 // It is then converted from radians to degrees
#ifdef RESTRICT_PITCH
double roll = atan2(accY, accZ) * RAD_TO_DEG;
double pitch = atan(-accX / sqrt(accY * accY + accZ * accZ)) * RAD_TO_DEG; 
#else double roll = atan(accY / sqrt(accX * accX + accZ * accZ)) * RAD_TO_DEG; 
...
void loop() 
{
  ...
  gyroZ = (i2cData[12] << 8) | i2cData[13]; // Calculate delta time 
...
delay(2); 
Serial.println(); // Corrected angles with offset
...
void pwm_adjust(int value_y)
{
  if(value_y >=-1 && value_y <=1 )
  {
    ...
    else if((value_y>=-3 && value_y<-1)||(value_y>1 && value_y<=3) )
    {
      ...
      {
        Serial.print("**");
        pwm_on = 120; // ms ON pwm_off = 3; // ms OFF 
        }
        else stop();
        }
